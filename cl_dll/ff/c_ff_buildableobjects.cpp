...
#if defined( CFFShield )
	#undef CFFShield
#endif

IMPLEMENT_CLIENTCLASS_DT( C_FFShield, DT_FFShield, CFFShield )
END_RECV_TABLE( )
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_FFShield::OnDataChanged( DataUpdateType_t updateType )
{
	// NOTE: We MUST call the base classes' implementation of this function
	BaseClass::OnDataChanged( updateType );
...
//-----------------------------------------------------------------------------
// Purpose: Creates a client side entity using the Shield model
//-----------------------------------------------------------------------------
C_FFShield *C_FFShield::CreateClientSideShield( const Vector& vecOrigin, const QAngle& vecAngles )
{
	C_FFShield *pShield = new C_FFShield;

	if( !pShield )
		return NULL;

	if( !pShield->InitializeAsClientEntity( FF_SHIELD_MODEL, RENDER_GROUP_OPAQUE_ENTITY ) )
	{
		pShield->Release( );
	...
	pShield->SetAbsOrigin( vecOrigin );
	pShield->SetLocalAngles( vecAngles );
	pShield->SetCollisionGroup( COLLISION_GROUP_BUILDABLE );
	pShield->SetRenderMode( kRenderTransAlpha );
	pShield->SetRenderColorA( ( byte )110 );

	if(ffdev_pulsebuildable.GetBool())
		pShield->m_nRenderFX = g_BuildableRenderFx;
	...
	// someone accesses the m_hOwner.Get() and wants to return something
	// that isn't NULL!
	pShield->m_hOwner = ( C_BaseEntity * )C_BasePlayer::GetLocalPlayer();
	pShield->SetClientSideOnly( true );
	pShield->SetNextClientThink( CLIENT_THINK_ALWAYS );
...
C_FFManCannon *C_FFManCannon::CreateClientSideManCannon( const Vector& vecOrigin, const QAngle& vecAngles )
{
	...
	pManCannon->m_nSkin = ( C_BasePlayer::GetLocalPlayer()->GetTeamNumber() - 1 );
