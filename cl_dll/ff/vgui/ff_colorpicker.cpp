...
namespace vgui
{
	FFColorPicker::FFColorPicker(Panel *parent, const char *panelName, Panel *pActionSignalTarget, bool bColorModeIntensity) : BaseClass(parent, panelName)
	{
		...
		if(bColorModeIntensity)
		{
			...
			kv->SetInt("Value", FFQuantityPanel::ITEM_COLOR_MODE_CUSTOM);
			m_pColorMode->AddItem("#GameUI_Custom", kv);
			kv->deleteThis();
			kv = new KeyValues("Stepped");
			kv->SetInt("Value", FFQuantityPanel::ITEM_COLOR_MODE_STEPPED);
			m_pColorMode->AddItem("#GameUI_Stepped", kv);
			kv->deleteThis();
			kv = new KeyValues("Faded");
			kv->SetInt("Value", FFQuantityPanel::ITEM_COLOR_MODE_FADED);
			m_pColorMode->AddItem("#GameUI_Faded", kv);
			kv->deleteThis();
			kv = new KeyValues("TeamColored");
			kv->SetInt("Value", FFQuantityPanel::ITEM_COLOR_MODE_TEAMCOLORED);
		...
		else
		{
			...
			kv->deleteThis();
			kv = new KeyValues("TeamColored");
			kv->SetInt("Value", FFQuantityPanel::COLOR_MODE_TEAMCOLORED);
			m_pColorMode->AddItem("#GameUI_TeamColored", kv);
			kv->deleteThis();
		}
	...
	void FFColorPicker::RecalculateColorPreview()
	{
		...
		{
			case FFQuantityPanel::ITEM_COLOR_MODE_CUSTOM:
				//custom
				SetBgColor( Color( m_iRed, m_iGreen, m_iBlue, m_iAlpha ) );
				break;
			case FFQuantityPanel::ITEM_COLOR_MODE_TEAMCOLORED:
				//teamcolored
				Color teamColor;
