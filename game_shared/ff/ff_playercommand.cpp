...
FF_SHARED_COMMAND( toggleone, &CFFPlayer::Command_ToggleOne, CC_ToggleOne, "Primes and throws primary grenade on subsequent touches", FF_CMD_ALIVE | FF_CMD_PREMATCH );
FF_SHARED_COMMAND( toggletwo, &CFFPlayer::Command_ToggleTwo, CC_ToggleTwo, "Primes and throws secondary grendae on subsequent touches", FF_CMD_ALIVE | FF_CMD_PREMATCH );
// <-- Mirv: Toggle grenades (requested by defrag)
...
FF_SHARED_COMMAND(throwgren, &CFFPlayer::Command_ThrowGren, CC_ThrowGren, "Throw a primed grenade.", FF_CMD_ALIVE | FF_CMD_PREMATCH);
//		had to do it this way because you cant use + and - with the macros
	...
	static CPlayerCommand SrvCmd_MinusGrenOne("-gren1", &CFFPlayer::Command_ThrowGren, FF_CMD_ALIVE | FF_CMD_PREMATCH);
	static CPlayerCommand SrvCmd_PlusGrenTwo("+gren2", &CFFPlayer::Command_PrimeTwo, FF_CMD_ALIVE | FF_CMD_PREMATCH);
	static CPlayerCommand SrvCmd_MinusGrenTwo("-gren2", &CFFPlayer::Command_ThrowGren, FF_CMD_ALIVE | FF_CMD_PREMATCH);
#else
