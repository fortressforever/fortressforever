...
BuildInfoResult_t CFFBuildableInfo::CanOrientToGround()
{
	...
	switch (m_iBuildObject)
	{
		case FF_BUILD_DISPENSER:
		{
			...
			for( int i = 0; i < 4; i++ )
			{
				UTIL_TraceLine( vecCorners[i], vecCorners[i] - Vector(0, 0, m_flTestDist), CONTENTS_SOLID|CONTENTS_MOVEABLE|CONTENTS_WINDOW|CONTENTS_MONSTER|CONTENTS_GRATE, m_pPlayer, COLLISION_GROUP_NONE, &tr[ i ] );
		...
		case FF_BUILD_SENTRYGUN:
		{
			...
			for( int i = 0; i < 3; i++ )
			{
				UTIL_TraceLine( vecFeet[i], vecFeet[i] - Vector(0, 0, m_flTestDist), CONTENTS_SOLID|CONTENTS_MOVEABLE|CONTENTS_WINDOW|CONTENTS_MONSTER|CONTENTS_GRATE, m_pPlayer, COLLISION_GROUP_NONE, &tr[ i ] );
