...
ConVar ffdev_slowfield_radius_inner("ffdev_slowfield_radius_inner", "176", FCVAR_REPLICATED/* | FCVAR_CHEAT */, "Inner radius of slowfield grenade (where slowfield has full effect)");
#define SLOWFIELD_RADIUS_INNER ffdev_slowfield_radius_inner.GetFloat()

ConVar ffdev_slowfield_radius_power("ffdev_slowfield_radius_power", "3", FCVAR_REPLICATED/* | FCVAR_CHEAT */, "Power by which to raise the pecentage that a player is between inner & outer radius of the slowfield grenade (so it ramps up and feels less like a brick wall)");
