...
#include "weapon_grenade.h"
	...
	#include "ff_player.h"
	#include "items.h"
	#include "ff_basegrenade_projectile.h"
...
IMPLEMENT_NETWORKCLASS_ALIASED( FFGrenade, DT_FFGrenade )

BEGIN_NETWORK_TABLE(CFFGrenade, DT_FFGrenade)
END_NETWORK_TABLE()

BEGIN_PREDICTION_DATA( CFFGrenade )
END_PREDICTION_DATA()

LINK_ENTITY_TO_CLASS( weapon_grenade, CFFGrenade );
...
BEGIN_DATADESC( CFFGrenade )
END_DATADESC()

void CFFGrenade::EmitGrenade( Vector vecSrc, QAngle vecAngles, Vector vecVel, AngularImpulse angImpulse, CBasePlayer *pPlayer )
{
