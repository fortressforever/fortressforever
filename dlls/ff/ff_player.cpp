...
ConVar ffdev_flamesize_burn2("ffdev_flamesize_burn2","0.030", FCVAR_REPLICATED, "flame size multiplier for burn level 2");
#define FFDEV_FLAMESIZE_BURN2 ffdev_flamesize_burn2.GetFloat()
ConVar ffdev_flamesize_burn3("ffdev_flamesize_burn3","0.045", FCVAR_REPLICATED, "flame size multiplier for burn level 3");
...
ConVar ffdev_infect_damage("ffdev_infect_damage","30",0,"Damage of the first infection tick (future ticks are modified by the infect_damagepertick_ vars)");
...
ConVar ffdev_infect_damagepertick_exp("ffdev_infect_damagepertick_exp",".8",0,"Infection tick damage is raised to this power each tick");
...
ConVar ffdev_infection_time( "ffdev_infection_time", "14", 0, "Amount of time for infection to last( in seconds )" );
...
ConVar ffdev_gibdamage("ffdev_gibdamage", "60", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_NOTIFY, "If a players health is -(ffdev_gibdamage's value) or less after death, then they will gib instead of ragdoll");
...
void CFFPlayer::JetpackSetFlame(bool bEmitting)
{
	if (bEmitting)
	{
		...
		if (!m_hJetpackFlame)
		{
			...
			m_hJetpackFlame = dynamic_cast<CFFJetpackFlame *> (CBaseEntity::Create("env_jetpackflame", Vector(0,0,0), QAngle(0,0,0), this));
			m_hJetpackFlame->SetAbsVelocity(Vector(0,0,-64));
			m_hJetpackFlame->SetAbsAngles(QAngle(0,0,-1));
			// Should inherit it's angles & position from the player for now
			//m_hFlameJet->SetOwnerEntity(this);
			//m_hFlameJet->FollowEntity(this);
		}

		m_hJetpackFlame->FlameEmit(true);
	}
	else
	{
		if (m_hJetpackFlame)
		{
			m_hJetpackFlame->FlameEmit( false );
			UTIL_Remove( m_hJetpackFlame );
		}
	}
}